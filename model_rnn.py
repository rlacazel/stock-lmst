...
class LstmRNN(object):
			...
			for label_, d_ in enumerate(dataset_list):
				for batch_X, batch_y in d_.generate_one_epoch(config.batch_size):
					...
					if np.mod(global_step, len(dataset_list) * 100 / config.input_size) == 1:
						...
						# Plot samples
						for sample_sym, indices in iter(sample_indices.items()):
	...
	def train_stock(self, dataset, config):
		"""
		Args:
			dataset_list (<StockDataSet>)
			config (tf.app.flags.FLAGS)
		...
		# num_batches = len(dataset.train_X)// config.batch_size
		random.seed(time.time())

		# Select samples for plotting.
		target_indices = np.array(range(0, len(dataset.test_X)))
			
			
		print("Start training for stocks:", dataset.stock_sym)
		for epoch in range(config.max_epoch):
			epoch_step = 0
			learning_rate = config.init_learning_rate * (
				config.learning_rate_decay ** max(float(epoch + 1 - config.init_epoch), 0.0)
			...
			# A batch is a subset of all inputs of size 'config.batch_size'
			for batch_X, batch_y in dataset.generate_one_epoch(config.batch_size):
				global_step += 1
				epoch_step += 1
				batch_labels = np.array([[0]] * len(batch_X))
				train_data_feed = {
					self.learning_rate: learning_rate,
					self.inputs: batch_X,
					self.targets: batch_y
				...
				# Plot every n steps
				if np.mod(global_step, 100 / config.input_size) == 1:
					test_loss, test_pred = self.sess.run([self.loss, self.pred], test_data_feed)
					...
					# Plot samples
					image_path = os.path.join(self.model_plots_dir, "{}_epoch{:02d}_step{:04d}.png".format(
						dataset.stock_sym, epoch, epoch_step))
					sample_preds = test_pred[target_indices]
					sample_truth = dataset.test_y[target_indices]
					self.plot_samples(sample_preds, sample_truth, image_path, stock_sym=dataset.stock_sym)
	...
	def plot_samples(self, preds, targets, figname, stock_sym=None):
		...
		#correct = [i for i in range(199,0,-1) if (truths[-i]-truths[-i-1])*(preds[-i]*preds[-i-1]) > 0]
		days = range(len(truths))[-200:]

		plt.figure(figsize=(12, 6))
		plt.plot(days, truths, label='truth')
		plt.plot(days, preds, label='pred')
		#plt.plot(days, correct, label='correct')
